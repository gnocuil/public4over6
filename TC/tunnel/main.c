...
{  
   ...
   if(mapping_num>0)
   {
   printf("%-16s%-40s%-28s%-10s%-10s%-20s%-10s%-20s","TI IPv4","TI IPv6","Start Time","Time Lmt","InPkts","InBytes","OutPkts","OutBytes");
   printf("%-10s\n","MapType");
   ...
   for(i=0;i<mapping_num;i++)
   {
      ...
      printf("%-16s%-40s%-4d/%02d/%-2d ",addr,addr6,(1900+p->tm_year),(1+p->tm_mon),p->tm_mday); 
      printf("%-3s %-2d:%-2d:%-7d%-10d",wday[p->tm_wday],p->tm_hour,p->tm_min,p->tm_sec,pecitem[i].seconds);
      printf("%-10lld%-20lld%-10lld%-20lld",pecitem[i].in_pkts,pecitem[i].inbound_bytes,pecitem[i].out_pkts,pecitem[i].outbound_bytes);
...
void set_mapping(struct in_addr remote,struct in6_addr remote6,int seconds)
{
   struct ecitem itm;
   struct ifreq req;
   int sock;
...
//del the ecitem
void del_mapping(struct in_addr remote)
{
   struct ecitem itm;
   struct ifreq req;
   int sock;
...
void use_info()
{
   printf("./ioctl [-h|-help] | [-a source_ipv4 source_ipv6 lease_time ] | [-b source_ipv4 ]\n");
...
int main(int argc,char *argv[])
{
    ...
    while(index<=argc)
    {
      ...
      else if(strncmp(argv[index-1],"-a",2)==0)//set mapping of source4 source6
      {
         ...
         seconds=atoi(argv[index+2]);
         inet_pton(AF_INET,argv[index],&saddr);//source ipv4 address
	     inet_pton(AF_INET6,argv[index+1],&saddr6);//source ipv6 address
         set_mapping(saddr,saddr6,seconds);
         break;
      }
