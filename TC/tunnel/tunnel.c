...
int public4over6_tunnel_xmit(struct sk_buff *skb,struct net_device *dev)
{
    ...
    ect=public4over6_ecitem_lookup(dev,(struct in_addr*)&daddr);
    if(ect)
    {
...
{  
   ...
   else if(cmd==TUNNEL_DEL_MAPPING)
   {
      ...
      pecit=public4over6_ecitem_unlink(netdev,&pecit->remote,1);//we just delete mapping item whose tag is 1(manual mapping item)
      if(pecit)
...
int public4over6_rcv(struct sk_buff *skb)
{
    ...
    {
       stats->rx_errors++;  
       CDBG("error in decapsulating a packet!\n");
       dev_put(ndev);
       goto rcv_error;
    }
    CDBG("public4over6_rcv: receiving a packet\n");
    stats->rx_packets++;
    ...
    iph=ip_hdr(skb);
    pect=public4over6_ecitem_lookup(netdev,(struct in_addr*)&iph->saddr);
    ptmp=public4over6_ecitem_lookup(netdev,(struct in_addr*)&iph->daddr);
